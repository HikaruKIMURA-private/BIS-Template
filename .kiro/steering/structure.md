...
**Location**: `auth.ts`（サーバー）, `libs/auth-client.ts`（クライアント）  
...
**Location**: `libs/`（アバターは `libs/avatar/`、ストレージは `libs/storage/`）  
**Purpose**: 共有ユーティリティ関数  
...
**Pattern**:
- Client Component: `app/components/Foo.test.tsx`
- Server Action: `app/actions/foo.test.ts`（DB / auth をモック）
- Data Fetching: `app/data/foo.test.ts`（DB をモック）
