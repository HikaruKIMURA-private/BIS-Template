...
### Stories (Component Catalog)

**Location**: `stories/`  
**Purpose**: Storybook ストーリーはコンポーネントカタログ + Chromatic VRT 専用。`pnpm test` からは除外。play 関数はUI状態の展示用のみ（テストアサーションは含めない）

### Unit / Integration Tests

**Location**: `*.test.{ts,tsx}`（ソースと同一ディレクトリにコロケーション）  
**Purpose**: Vitest + Testing Library による単体テスト・結合テスト  
**Pattern**:
- Client Component: `app/components/Foo.test.tsx`（jsdom）
- Server Action / Data: `app/actions/foo.test.ts`, `app/data/foo.test.ts`（Vitest node + 実 DB `postgres_test`）。auth / `next/*` 等のプロセス外依存のみモックし、DB は実体を使う（詳細は `AGENTS.md`）
- DB エラーなど実 DB では再現しづらいケース: `*.error.test.ts` でモック等を使い分け
...
- **Unit/Integration テスト**: `*.test.{ts,tsx}`（ソースと同一ディレクトリにコロケーション）
- **E2E テスト**: `*.spec.ts`（`e2e/` ディレクトリに配置）
- **Storybook**: `*.stories.tsx`（`stories/` ディレクトリに配置、カタログ専用）
