...
テストしにくいコードはモックを増やすのではなく、コード構造を改善して解決する。

- **純粋関数を優先**: バリデーション・データ変換・ビジネスルールは副作用のない純粋関数として切り出す
- **副作用を境界に押し出す**: DB 操作・リダイレクト・キャッシュ無効化は関数の最外層にまとめる。内部ロジックは入力→出力の変換に徹する
- **1関数1責務**: バリデーション、変換、永続化を1つの関数に詰め込まない。分離すれば個別にテストできる
- **コンポーネント = 表示、hooks = ロジック**: UI コンポーネントから状態管理・計算ロジックを hooks に抽出し、それぞれ独立してテスト可能にする
- **依存は引数で渡す**: テスト時に差し替え可能な設計。グローバル状態への直接依存を避ける

## 禁止パターン

- テストケースとテスト実装を同時に書く（ケース列挙が先）
- テストしにくいコードにモックを増やして対処する（コード設計を改善すること）
